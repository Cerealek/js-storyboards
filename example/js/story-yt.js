function startStory() {
	...
	timelines[0] = new Timeline('youtube');
	timelines[0].loopAction = function() {  $('#timer').html(Math.floor(this.currentTime() / 1000)); }
	timelines[0].attachedTo = 'ytplayer_object';
	$('#theater').html('');
	$('#theater').css('backgroundColor', 'black'); 
	                                         
	// adding actions, in this case timed lyrics	                                         
	                                         
	timelines[0].addAction(new Action(4000,  function() { $('#theater').html('I see your face every morning'); ;  }));	
	timelines[0].addAction(new Action(9700, function() { $('#theater').html('I watch the years passing by'); }));	
	timelines[0].addAction(new Action(16000, function() { $('#theater').html('Dye your hair yellow dye it violet'); }));	
	timelines[0].addAction(new Action(21000, function() { $('#theater').html('Whatever colour you might like'); }));	
  timelines[0].addAction(new Action(27700, function() { $('#theater').html('Still I know what’s behind it'); }));	
  timelines[0].addAction(new Action(33500, function() { $('#theater').html('I know the colour you trying to hide'); }));
	
	// you can easily stop the timeline and make it start again; when it's an audio element, it will restart from the beginning	
	timelines[0].addAction(new Action(39000, function() { $('#theater').html(''); timelines[0].stopTimeline(); timelines[0].startTimeline();  }));
